...
{
    private static ?OPTISTATE_DB_Wrapper $instance = null;
    private ?mysqli $connection = null;
    private int $max_retries = 3;
    private ?array $db_config_cache = null;
    private ?int $connection_created_at = null;
    private int $query_count = 0;
    private int $total_queries_executed = 0;
    private int $connection_errors = 0;
    private const MAX_QUERY_COUNT = 10000;
    private const MAX_CONNECTION_AGE = 300;
    ...
    public static function get_instance(): self
    {
        ...
        if (self::$instance->should_refresh_connection()) {
            self::$instance->refresh_connection();
        }
    ...
    private function should_refresh_connection(): bool
    {
        ...
        if ($this->query_count >= self::MAX_QUERY_COUNT) {
            return true;
        }
        if (
            $this->connection_created_at &&
            time() - $this->connection_created_at >= self::MAX_CONNECTION_AGE
        ) {
            return true;
        }
        if ($this->last_ping === null || time() - $this->last_ping > 30) {
    ...
    public function get_connection(): mysqli
    {
        ...
        if ($this->db_config_cache === null) {
            ...
            $db_pass = !empty($wpdb->dbpassword)
                ? $wpdb->dbpassword
                : DB_PASSWORD;
            $db_name = !empty($wpdb->dbname) ? $wpdb->dbname : DB_NAME;
            $port = null;
            $socket = null;
        ...
        while ($attempts < $this->max_retries) {
            try {
                ...
                if (defined("MYSQLI_OPT_READ_TIMEOUT")) {
                    $mysqli->options(MYSQLI_OPT_READ_TIMEOUT, $timeout * 2);
                }
                if (defined("MYSQLI_OPT_WRITE_TIMEOUT")) {
                    $mysqli->options(MYSQLI_OPT_WRITE_TIMEOUT, $timeout * 2);
                }
                ...
                if (defined("MYSQL_SSL_CA") && MYSQL_SSL_CA) {
                    $mysqli->ssl_set(
                        defined("MYSQL_SSL_KEY") ? MYSQL_SSL_KEY : null,
                        defined("MYSQL_SSL_CERT") ? MYSQL_SSL_CERT : null,
                        MYSQL_SSL_CA,
                        defined("MYSQL_SSL_CAPATH") ? MYSQL_SSL_CAPATH : null,
                        defined("MYSQL_SSL_CIPHER") ? MYSQL_SSL_CIPHER : null
                    );
                }
                try {
                    $connected = $mysqli->real_connect(
                        $db_host,
                        $this->db_config_cache["user"],
                        $this->db_config_cache["pass"],
                        $this->db_config_cache["name"],
                        $port,
                        $socket,
                        $client_flags
                    );
                ...
                if ($connected) {
                    $this->apply_default_session_settings($mysqli);
                ...
                if ($attempts < $this->max_retries) {
                    $is_web_request = !defined("WP_CLI") && !wp_doing_cron();
                    if ($is_web_request) {
                        $base_delay = 200000 * $attempts;
                        $jitter = wp_rand(0, 100000);
                    } else {
                        $base_delay = (int) (pow(2, $attempts - 1) * 1000000);
                        $jitter = wp_rand(0, 1000000);
                    }
        ...
        OPTISTATE_Utils::log_critical_error(
            ...
            [
                "host" => $db_host,
                "user" => $this->db_config_cache["user"],
                "db" => $this->db_config_cache["name"],
        ...
        throw new Exception(
            sprintf(
                __(
                    "Failed to connect to database after %s attempts. Error: %s",
                    "optistate"
                ),
                number_format_i18n($this->max_retries),
                $last_error
            )
        );
    }
        ...
        if ($wp_timezone) {
            $connection->query(
                "SET SESSION time_zone = '" .
                    $connection->real_escape_string($wp_timezone) .
                    "'"
            );
        }
    ...
    private function get_wp_timezone(): ?string
    {
        ...
        global $wpdb;
        $wp_timezone = $wpdb->get_var("SELECT @@session.time_zone");
        $this->cached_wp_timezone =
            $wp_timezone !== null && $wp_timezone !== "" ? $wp_timezone : null;
    ...
    public function force_commit_if_needed(): bool
    {
        if (
            ...
            $this->query_count < self::MAX_QUERY_COUNT - 100
        ) {
            return true;
        }
        ...
        $newTx = $this->begin_transaction();
        if ($newTx === false) {
            OPTISTATE_Utils::log_critical_error(
                "force_commit_if_needed: failed to restart transaction"
            );
            return false;
        }
        return true;
    ...
    public function query(string $query)
    {
        ...
        $is_lock_query = (bool) preg_match(
            "/\bGET_LOCK\b|\bRELEASE_LOCK\b/i",
            $query
        );
        if ($is_lock_query) {
            $this->track_lock_operation($query);
        }
        $this->query_count++;
        ...
        if ($result === false) {
            ...
            if (
                in_array($errno, [2006, 2013], true) &&
                empty($this->active_locks)
            ) {
                ...
                if ($result === false) {
                    OPTISTATE_Utils::log_critical_error(
                        "Query failed after connection retry",
                        [
                            "errno" => $errno,
                            "query_preview" => substr($query, 0, 200),
                            "error" => $connection->error,
                            "in_transaction" => $this->transaction_active,
                            "active_locks" => array_keys($this->active_locks),
                            "connection_id" => $connection->thread_id ?? null,
                        ]
                    );
                }
            } else {
                OPTISTATE_Utils::log_critical_error("Query execution failed", [
                    "errno" => $errno,
                    "query_preview" => substr($query, 0, 200),
                    "error" => $connection->error,
                    "in_transaction" => $this->transaction_active,
    ...
    private function track_lock_operation(string $query): void
    {
        ...
        if (
            preg_match(
                "/SELECT\s+GET_LOCK\s*\(\s*['\"]([^'\"]+)['\"]\s*,\s*(\d+)\s*\)/i",
                $query,
                $matches
            )
        ) {
            ...
            if (isset($this->active_locks[$lock_name])) {
                $this->active_locks[$lock_name]["pending_release"] = true;
            }
        }
    }
    private function update_lock_status(string $query, $result): void
    {
        ...
        if (
            ...
            if ($result instanceof mysqli_result) {
                $row = $result->fetch_row();
                $lock_result = $row ? (int) $row[0] : 0;
                $result->free();
                if ($lock_result === 1) {
                    if (isset($this->active_locks[$lock_name])) {
                        $this->active_locks[$lock_name]["lock_result"] = 1;
                    }
                } else {
            ...
            if ($result instanceof mysqli_result) {
                $row = $result->fetch_row();
                $release_result =
                    $row !== null && $row !== false ? (int) $row[0] : 0;
                $result->free();
                if ($release_result === 1) {
                    unset($this->active_locks[$lock_name]);
                } else {
                    OPTISTATE_Utils::log_critical_error(
                        ...
                        ["lock_name" => $lock_name, "result" => $release_result]
                    );
                }
            } else {
                unset($this->active_locks[$lock_name]);
                OPTISTATE_Utils::log_critical_error(
                    "RELEASE_LOCK query failed outright",
                    ["lock_name" => $lock_name]
                );
            }
        }
    ...
    public function begin_transaction()
    {
        ...
        if ($result) {
            ...
            $this->transaction_start_time = time();
            return true;
        }
        OPTISTATE_Utils::log_critical_error("Failed to begin transaction", [
            "error" => $connection->error,
        ]);
        return false;
    }
    public function commit(): bool
    {
        ...
        OPTISTATE_Utils::log_critical_error("Failed to commit transaction", [
            "error" => $connection->error,
            "queries_in_transaction" => $this->query_count,
        ]);
        return false;
    }
    public function rollback($savepoint = null): bool
    {
        ...
        $connection = $this->get_connection();
        if ($savepoint !== null) {
            $result = $this->query("ROLLBACK TO SAVEPOINT {$savepoint}");
            if ($result !== false) {
                return true;
            }
            OPTISTATE_Utils::log_critical_error(
            ...
            if (!empty($this->active_locks)) {
                foreach (array_keys($this->active_locks) as $lock_name) {
                    $release_query = sprintf(
                        ...
                        $this->connection->real_escape_string($lock_name)
                    );
                    $result = @$this->connection->query($release_query);
                    if ($result === false) {
                        OPTISTATE_Utils::log_critical_error(
                            "Failed to release lock on close",
                            ["lock_name" => $lock_name]
                        );
                    }
                }
    ...
    public function emergency_cleanup(): void
    {
        if ($this->transaction_active || !empty($this->active_locks)) {
            try {
                if ($this->connection !== null) {
                    $is_alive = $this->is_connection_alive();
                    if ($is_alive) {
                        if ($this->transaction_active) {
                            @$this->connection->rollback();
                            @$this->connection->autocommit(true);
