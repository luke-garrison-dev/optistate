...
class OPTISTATE_Cleanup_Functions
{
    ...
    private function run_batch_delete(
        ...
        do {
            ...
            if ($last_elapsed !== null && $last_elapsed > 2) {
    ...
    private function clean_action_scheduler(): int
    {
        ...
        $clean_orphans = function (
            ...
            while (true) {
                ...
                $ids = $wpdb->get_col(
                    $wpdb->prepare(
                        "SELECT c.{$child_id_col}
                         FROM {$child_table} c
                         LEFT JOIN {$table_actions} a
                                ON a.{$parent_id_col} = c.{$parent_id_col}
                         WHERE a.{$parent_id_col} IS NULL
                         ORDER BY c.{$child_id_col} ASC
                         LIMIT %d",
                        $batch_size
                    )
                );
                ...
                try {
                    ...
                    OPTISTATE_Utils::log_critical_error(
                        ...
                        ["table" => $child_table, "error" => $e->getMessage()]
                    );
                    break;
                }
    ...
    private function clean_woocommerce_bloat(): int
    {
            ...
            do {
                ...
                $candidates = $wpdb->get_col(
                    $wpdb->prepare(
                        ...
                        $exclude_timeout_like,
                        $exclude_expires_like,
                        $batch_size
                    )
                );
                ...
                $timeout_candidates = array_map(
                    fn($name) => str_replace($p["value"], $p["timeout"], $name),
                    $candidates
                );
        ...
        if (OPTISTATE_Utils::table_exists($wc_session_table)) {
            ...
            } while ($deleted !== false && $deleted >= $batch_size);
        }
        wp_cache_delete("alloptions", "options");
        wp_cache_delete("notoptions", "options");
                ...
                try {
                    $tt_deleted_count = OPTISTATE_Utils::transaction(
                        function () use (
                            ...
                            if ($deleted === false) {
                                throw new \Exception(
                                    "term_taxonomy delete failed for orphan taxonomy batch"
                                );
                            }
                                    ...
                                    $wpdb->query(
                                        $wpdb->prepare(
                                            "DELETE FROM {$wpdb->terms}
                                             WHERE term_id IN ($delete_placeholders)",
                                            ...$term_ids_to_delete
                                        )
                                    );
                                    $wpdb->query(
                    ...
                    $count += $tt_deleted_count;
                } catch (\Throwable $e) {
                    OPTISTATE_Utils::log_critical_error(
                        "clean_empty_taxonomies orphan batch interrupted",
                        ["error" => $e->getMessage()]
                    );
                }
            }
    ...
    private function clean_trashed_posts(): int
    {
        ...
        do {
            ...
            $touched_taxonomies = $wpdb->get_col(
                "SELECT DISTINCT tt.taxonomy
                 ...
                 INNER JOIN {$wpdb->term_taxonomy} tt
                         ON tr.term_taxonomy_id = tt.term_taxonomy_id
                 WHERE tr.object_id IN ($safe_ids)"
            );
            ...
            $all_comment_ids = $this->collect_comment_tree_for_posts($safe_ids);
    ...
    private function collect_comment_tree_for_posts(string $safe_post_ids): array
    {
        ...
        $root_ids = $wpdb->get_col(
            "SELECT comment_ID
             FROM {$wpdb->comments}
             WHERE comment_post_ID IN ($safe_post_ids)"
        );
        ...
        while (!empty($frontier) && $depth < $max_depth && count($all_cids) < $max_ids) {
            ...
            $next_level = array_values(array_diff($next_level, $all_cids));
        ...
        do {
            ...
            $ids = $wpdb->get_col(
                $wpdb->prepare(
                    "SELECT comment_ID
                     ...
                     LIMIT %d",
                    ...array_merge($where_params, [$last_id, $batch_size])
                )
            );
            ...
            try {
                $deleted = OPTISTATE_Utils::transaction(function () use ($wpdb, $safe_ids) {
                    ...
                    $deleted = $wpdb->query(
                        "DELETE FROM {$wpdb->comments} WHERE comment_ID IN ($safe_ids)"
                    );
                    if ($deleted === false) {
                        throw new \Exception("comments delete failed");
                    }
    ...
    {
        return $this->clean_comments_by_condition("comment_approved = 'spam'");
    }

    private function clean_trashed_comments(): int
    {
        return $this->clean_comments_by_condition("comment_approved = 'trash'");
    }

    private function clean_unapproved_comments(): int
    {
        return $this->clean_comments_by_condition("comment_approved = '0'");
    }

    private function clean_pingbacks(): int
    {
        return $this->clean_comments_by_condition("comment_type = 'pingback'");
    }

    private function clean_trackbacks(): int
    {
        return $this->clean_comments_by_condition("comment_type = 'trackback'");
    }
    private function clean_orphaned_meta(
        ...
        while (true) {
            ...
            $rows = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT m.{$meta_id_col}, m.{$meta_fk_col}
                     FROM {$meta_table} m
                     LEFT JOIN {$parent_table} p
                            ON p.{$parent_id_col} = m.{$meta_fk_col}
                     WHERE p.{$parent_id_col} IS NULL
                       AND m.{$meta_id_col} > %d
                     ORDER BY m.{$meta_id_col} ASC
                     LIMIT %d",
                    $last_meta_id,
                    $batch_size
                ),
            ...
            $last_meta_id = (int) end($orphaned_ids);
                ...
                $wpdb->prepare(
                    "SELECT tr.object_id, tr.term_taxonomy_id, tt.term_id, tt.taxonomy
                     ...
                     LEFT JOIN {$wpdb->users} u
                            ON u.ID = tr.object_id
                     {$links_join}
                     WHERE tt.term_taxonomy_id IS NULL
                        OR (p.ID IS NULL AND u.ID IS NULL {$links_null_check})
                ...
                $wpdb->prepare(
                    "SELECT option_id, option_name
                     ...
                     WHERE (option_name LIKE %s OR option_name LIKE %s)
                       AND option_name NOT LIKE '\_transient\_wc\_%%'
                       AND option_name NOT LIKE '\_transient\_oembed\_%%'
                       AND option_id > %d
                     ORDER BY option_id ASC
                     LIMIT %d",
                    $like_transient,
    ...
    private function clean_duplicate_meta(
        ...
        $table_status = OPTISTATE_Utils::get_table_status($table);
        $row_estimate = (int) ($table_status["TABLE_ROWS"] ?? 0);

        if ($max_rows > 0 && $row_estimate > $max_rows) {
            OPTISTATE_Utils::log_critical_error(
        ...
        while (true) {
            ...
            $duplicate_ids = $wpdb->get_col(
                $wpdb->prepare(
                    "SELECT /*+ MAX_EXECUTION_TIME(60000) */ {$meta_id_col}
                     FROM (
                         SELECT
                             {$meta_id_col},
                             ROW_NUMBER() OVER (
                                 PARTITION BY {$group_col}, meta_key, MD5(meta_value)
                                 ORDER BY {$meta_id_col} ASC
                             ) AS rn,
                             COUNT(*) OVER (
                                 PARTITION BY {$group_col}, meta_key, MD5(meta_value)
                             ) AS grp_cnt
