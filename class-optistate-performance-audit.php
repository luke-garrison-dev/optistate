...
class OPTISTATE_Performance_Audit {
	...
	public function __construct( OPTISTATE $main_plugin ) {
		...
		add_action( 'wp_ajax_nopriv_optistate_run_pagespeed_worker_async', [ $this, 'ajax_run_pagespeed_worker_async' ] );
	}
	...
	public function run_pagespeed_worker( string $task_id ): void {
		...
		try {
			...
			$wpdb->delete( $wpdb->options, [ 'option_name' => $mutex_option ], [ '%s' ] );
		}
	}
	...
	public function ajax_run_pagespeed_audit(): void {
		...
		try {
			...
			$scheduled = wp_schedule_single_event( time() + 1, 'optistate_run_pagespeed_worker', [ $task_id ] );
			if ( ! $scheduled ) {
				OPTISTATE_Utils::log_critical_error(
	...
	public function ajax_check_pagespeed_status(): void {
		...
		if (
			...
			$this->main_plugin->process_store->set( $task_id, $task, 300 );
		}
		...
		if ( $task['status'] === 'done' ) {
			...
			OPTISTATE_Utils::send_json_success( [ 'status' => $task['status'] ] );
		}
	}
