...
class OPTISTATE_Performance_Audit
{
    ...
    public function run_pagespeed_worker(string $task_id): void
    {
        ...
        if (!is_array($data)) {
            $task["status"] = "error";
            $task["message"] = __(
                "Invalid API response (non-JSON body).",
                "optistate"
            );
            $process_store->set($task_id, $task, 600);
            return;
    ...
    public function ajax_run_pagespeed_audit(): void
    {
        ...
        if (!$is_valid_domain) {
            OPTISTATE_Utils::send_json_error(
                __(
                    "Security Restriction: You can only test URLs belonging to this domain or its subdomains.",
                    "optistate"
                )
            );
            return;
        ...
        try {
            ...
            $process_store = $this->main_plugin->process_store->set(
                $task_id,
                $task_data,
                600
            );
    ...
    public function ajax_check_pagespeed_status(): void
    {
        ...
        if (
            ...
            $cache_key =
                "optistate_pagespeed_" . md5($task["url"] . $task["strategy"]);
