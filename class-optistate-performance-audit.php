...
class OPTISTATE_Performance_Audit
{
    ...
    public function __construct(OPTISTATE $main_plugin)
    {
        ...
        add_action("wp_ajax_nopriv_optistate_run_pagespeed_worker_async", [
            $this,
            "ajax_run_pagespeed_worker_async",
        ]);
    }
    ...
    public function run_pagespeed_worker(string $task_id): void
    {
        ...
        global $wpdb;
        $mutex_option = "optistate_psi_wrk_" . $task_id;
        $mutex_expiry = time() + 120;
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$wpdb->options} WHERE option_name = %s AND CAST(option_value AS SIGNED) < %d",
                $mutex_option,
                time()
            )
        );
            ...
            try {
                ...
                do {
                    ...
                    $response = wp_remote_get($endpoint, [
                        ...
                        "user-agent" => "OptiState-PageSpeed-Audit/1.0",
                    ]);
                    ...
                    $is_retryable =
                        stripos($last_error_msg, "timed out") !== false ||
                        stripos($last_error_msg, "timeout") !== false ||
                        stripos($last_error_msg, "cURL error 28") !== false ||
                        stripos($last_error_msg, "cURL error 6") !== false ||
                        stripos($last_error_msg, "cURL error 7") !== false ||
                        stripos($last_error_msg, "cURL error 35") !== false ||
                        stripos($last_error_msg, "cURL error 52") !== false ||
                        stripos($last_error_msg, "cURL error 56") !== false;
                ...
                $features_status = [
                    "server_caching" => !empty(
                        $perf_settings["server_caching"]["enabled"]
                    ),
                    "browser_caching" => !empty(
                        $perf_settings["browser_caching"]
                    ),
                    "lazy_load" => !empty($perf_settings["lazy_load"]),
                    "db_query_caching" => !empty(
                        $perf_settings["db_query_caching"]["enabled"]
                    ),
                    "font_optimization" => !empty(
                        $perf_settings["font_optimization"]["enabled"]
                    ),
                    "font_display_swap" =>
                        !empty(
                            $perf_settings["font_optimization"]["enabled"]
                        ...
                        !empty(
                            $perf_settings["font_optimization"]["display_swap"]
                        ),
                    "font_preconnect" =>
                        !empty(
                            $perf_settings["font_optimization"]["enabled"]
                        ...
                        !empty(
                            $perf_settings["font_optimization"]["preconnect"]
                        ),
                    "font_async" =>
                        !empty(
                            $perf_settings["font_optimization"]["enabled"]
                        ...
                        !empty(
                            $perf_settings["font_optimization"][
                                "async_google_fonts"
                            ]
                        ),
                    "heartbeat_api" =>
                    ...
                    "bad_bot_blocker" => !empty(
                        $perf_settings["bad_bot_blocker"]["enabled"]
                    ),
                ...
                if (isset($audits["server-response-time"])) {
                    ...
                    if (
                        isset($srt_audit["numericValue"]) &&
                        $srt_audit["numericValue"] > 600
                    ) {
                ...
                if (
                    isset($audits["offscreen-images"]) ||
                    isset($audits["modern-image-formats"])
                ) {
                    ...
                    if (
                        ...
                        if (!$features_status["lazy_load"]) {
                            $offscreen_count = isset(
                                $offscreen["details"]["items"]
                            )
                ...
                if (isset($audits["unused-css-rules"])) {
                    ...
                    if (isset($uc_audit["score"]) && $uc_audit["score"] < 0.9) {
                        $wasted_css = isset(
                            $uc_audit["details"]["overallSavingsBytes"]
                        )
                            ? round(
                                $uc_audit["details"]["overallSavingsBytes"] /
                                    1024
                ...
                if (isset($audits["unminified-javascript"])) {
                    ...
                    if (
                        isset($minify_audit["score"]) &&
                        $minify_audit["score"] < 0.9
                    ) {
                        $savings = isset(
                            $minify_audit["details"]["overallSavingsBytes"]
                        )
                            ? round(
                                $minify_audit["details"][
                                    "overallSavingsBytes"
                                ] / 1024
                            )
                            : 0;
                ...
                if (isset($audits["largest-contentful-paint-element"])) {
                    ...
                    if ($lcp_value > 2500) {
                        $lcp_element = isset(
                            $lcp_audit["details"]["items"][0]["node"][
                                "nodeLabel"
                            ]
                        )
                            ? $lcp_audit["details"]["items"][0]["node"][
                ...
                if (isset($audits["font-display"])) {
                    ...
                    if (
                        ...
                        if (!$features_status["font_optimization"]) {
                            ...
                            if (!$features_status["font_display_swap"]) {
                                $recommendations[] = [
                                    "priority" => "medium",
                                    "icon" => "dashicons-editor-textcolor",
                                    "title" => __(
                                        "Enable font-display: swap in Font Optimization",
                                        "optistate"
                                    ),
                                    "description" => __(
                            ...
                            if (!$features_status["font_async"]) {
                                $recommendations[] = [
                                    ...
                                    "description" => __(
                                        'Google Fonts are blocking text rendering. Enable the "Async Google Fonts" sub‑option in Font Loading Optimization to load fonts in the background, improving FCP.',
                                        "optistate"
                                    ),
                                    "tab" => "#tab-performance",
                                    "feature" => "font_optimization",
                            ...
                            if (!$features_status["font_preconnect"]) {
                                $recommendations[] = [
                                    "priority" => "medium",
                                    "icon" => "dashicons-external",
                                    "title" => __(
                                        "Enable Preconnect to Google Fonts CDN",
                                        "optistate"
                                    ),
                                    "description" => __(
                                        'Enable the "Preconnect" sub‑option in Font Loading Optimization to establish an early connection to fonts.gstatic.com, reducing latency and improving font delivery.',
                                        "optistate"
                                    ),
                                    "tab" => "#tab-performance",
                                    "feature" => "font_optimization",
                ...
                if (isset($audits["render-blocking-resources"])) {
                    ...
                    if (
                        $has_google_font &&
                        $features_status["font_optimization"] &&
                        !$features_status["font_async"]
                    ) {
                        $already_has_async = false;
                        foreach ($recommendations as $rec) {
                            if (
                                isset($rec["feature"]) &&
                                $rec["feature"] === "font_optimization" &&
                                strpos($rec["title"], "Async") !== false
                            ) {
                                $already_has_async = true;
                                break;
                            }
                        }
                        if (!$already_has_async) {
                            $recommendations[] = [
                                "priority" => "high",
                                "icon" => "dashicons-editor-textcolor",
                                "title" => __(
                                ...
                                "description" => __(
                                    'A Google Font stylesheet is blocking page rendering. Enable the "Async Google Fonts" sub‑option in Font Loading Optimization to load fonts in the background, improving FCP.',
                                    "optistate"
                                ),
                                "tab" => "#tab-performance",
                                "feature" => "font_optimization",
                ...
                usort($recommendations, static function ($a, $b) {
                    ...
                    return $priority_order[$a["priority"]] <=>
                        $priority_order[$b["priority"]];
                });
                if (!isset($ttfb_display)) {
                    $ttfb_display = "N/A";
                }
                ...
                $cache_key =
                    "optistate_pagespeed_" . md5($test_url . $strategy);
                ...
                OPTISTATE_Utils::log_critical_error(
                    "run_pagespeed_worker failed unexpectedly: " .
                        $e->getMessage(),
                    [
                        "task_id" => $task_id,
                        "file" => $e->getFile(),
    ...
    private function format_file_list(array $files): string
    {
        ...
        return "<br>" .
            esc_html__("• Affected files:", "optistate") .
            " " .
            implode(", ", $escaped) .
            ".";
    }
    ...
    public function ajax_run_pagespeed_audit(): void
    {
        ...
        $cached_only =
            isset($_POST["cached_only"]) && $_POST["cached_only"] === "true";
        $strategy =
            isset($_POST["strategy"]) && $_POST["strategy"] === "desktop"
                ? "desktop"
                : "mobile";
        $raw_url =
            isset($_POST["test_url"]) && is_string($_POST["test_url"])
        ...
        if (empty($test_url) && !$force_refresh) {
            ...
            if (is_array($last_state) && !empty($last_state["url"])) {
                ...
                $strategy = isset($last_state["strategy"])
                    ? $last_state["strategy"]
                    : $strategy;
            }
        }
        ...
        if (
            !isset($test_url_parsed["host"]) ||
            !isset($home_url_parsed["host"])
        ) {
            OPTISTATE_Utils::send_json_error(
                __("Invalid URL format.", "optistate")
            );
            return;
        }

        $test_host = wp_parse_url($test_url, PHP_URL_HOST);
        $home_host = wp_parse_url(home_url(), PHP_URL_HOST);
        $clean_test_host = preg_replace("/^www\./i", "", (string) $test_host);
        $clean_home_host = preg_replace("/^www\./i", "", (string) $home_host);
        $is_valid_domain =
            $clean_test_host === $clean_home_host ||
            substr($clean_test_host, -strlen("." . $clean_home_host)) ===
                "." . $clean_home_host;

        if (!$is_valid_domain) {
            OPTISTATE_Utils::send_json_error(
                __(
                    "Security Restriction: Custom URL must belong to this domain or its subdomains.",
                    "optistate"
                )
            );
            return;
        ...
        try {
            ...
            if (!$scheduled) {
                OPTISTATE_Utils::log_critical_error(
                    "Could not schedule pagespeed worker cron event, relying solely on async request.",
                    ["task_id" => $task_id]
                );
            ...
            OPTISTATE_Utils::log_critical_error(
                "ajax_run_pagespeed_audit: failed to launch background task: " .
                    $e->getMessage(),
    ...
    public function ajax_check_pagespeed_status()
    {
        ...
        if (empty($task_id)) {
            OPTISTATE_Utils::send_json_error(
                __("Validation Error: Missing task ID.", "optistate")
            );
            return;
        }
        ...
        if (!$task) {
            OPTISTATE_Utils::send_json_error(
                __("Execution Error: Task not found or expired.", "optistate")
            );
            return;
        }

        $elapsed = isset($task["started"])
            ? time() - (int) $task["started"]
            : 0;
        $is_stalled = $task["status"] === "processing" && $elapsed > 30;
        $is_abandoned = $task["status"] === "pending" && $elapsed > 30;
        if ($is_stalled || $is_abandoned) {
            global $wpdb;
            $wpdb->delete(
                $wpdb->options,
        ...
        if ($task["status"] !== "done" && $task["status"] !== "error") {
            if (
                ...
                $task["status"] = "error";
                $task["message"] = __(
                    "Timeout Error: The PageSpeed audit exceeded the 3-minute execution limit.",
                    "optistate"
                );
                $this->main_plugin->process_store->set($task_id, $task, 600);
                OPTISTATE_Utils::send_json_success([
            ...
            OPTISTATE_Utils::send_json_success([
                "status" => "processing",
                "message" => __("Auditing...", "optistate"),
            ]);
            return;
        }

        if ($task["status"] === "error") {
            $error_message = isset($task["message"])
                ? $task["message"]
                : (isset($task["error"])
                    ? $task["error"]
                    : __(
                        "API Error: The PageSpeed audit failed unexpectedly.",
                        "optistate"
                    ));
            OPTISTATE_Utils::send_json_success([
                "status" => "error",
        ...
        OPTISTATE_Utils::send_json_success([
            "status" => "done",
            "data" => isset($task["results"]) ? $task["results"] : [],
        ]);
    }
    ...
    public function ajax_run_pagespeed_worker_async(): void
    {
        ...
        if (!preg_match('/^psi_[a-f0-9]{16}$/', $task_id)) {
            wp_die("", "", ["response" => 400]);
        }
        $posted_token = isset($_POST["worker_token"])
            ? sanitize_text_field(wp_unslash($_POST["worker_token"]))
            : "";
        ...
        if (
            !$task ||
            !isset($task["worker_token"]) ||
            !hash_equals($task["worker_token"], $posted_token)
        ) {
            wp_die("", "", ["response" => 403]);
        }
        unset($task["worker_token"]);
        $this->main_plugin->process_store->set($task_id, $task, 600);
