...
class OPTISTATE_Settings_Manager
{
    ...
    {
        return (int) get_option("optistate_settings_version", 1);
    }
    private function increment_settings_version(): void
    {
        $new_version = $this->get_settings_version() + 1;
        update_option("optistate_settings_version", $new_version, true);
        wp_cache_delete(
            "optistate_settings_rows_v" . ($new_version - 1),
            "optistate"
        );
    }
    public function get_persistent_settings(): array
    {
        if ($this->settings_cache !== null) {
            return $this->settings_cache;
        }
        $version = $this->get_settings_version();
        $cache_key = "optistate_settings_rows_v" . $version;
        $cached = wp_cache_get($cache_key, "optistate");
        if (is_array($cached)) {
            $this->settings_cache = $cached;
            return $cached;
        }
        $option_settings = get_option(self::SETTINGS_CACHED_OPTION);
        ...
        $stored_raw = $this->main_plugin->get_store_data("settings", []);
        if (!is_array($stored_raw)) {
            $stored_raw = [];
        }
        $defaults = $this->get_default_settings();
        $settings = array_merge($defaults, $stored_raw);
        $validated = $this->validate_settings($settings);
        if ($settings !== $validated || empty($stored_raw)) {
    ...
    public function save_persistent_settings(array $new_settings): bool
    {
        try {
            ...
            if (
                ...
                get_option(self::SETTINGS_CACHED_OPTION) !== $validated
            ) {
                OPTISTATE_Utils::log_critical_error(
            ...
            OPTISTATE_Utils::log_critical_error(
                "save_persistent_settings failed: " . $e->getMessage(),
    ...
    public function get_default_settings(): array
    {
        ...
        $defaults = [
            ...
            "allowed_users" => [],
            "pagespeed_api_key" => OPTISTATE_Utils::encrypt_data(
                "AIzaSyDGo4ufkdrd2e8za0IRz_qnX-GWQdaBA2s"
            ),
    ...
    private function validate_settings(array $settings): array
    {
        ...
        foreach ($defaults as $key => $default_value) {
            ...
            switch ($key) {
                case "max_backups":
                    $validated[$key] = max(1, min(10, absint($value)));
                    break;
                case "auto_optimize_days":
                    $validated[$key] = max(0, min(365, absint($value)));
                    break;
                case "auto_optimize_time":
                    $validated[$key] = preg_match(
                        ...
                        : "02:00";
                    break;
                case "one_click_backup":
                    $validated[$key] = (bool) $value;
                    break;
                case "last_applied_preset":
                    $validated[$key] = is_string($value)
                        ? sanitize_key($value)
                        : "";
                    break;
                ...
                case "enable_two_factor":
                    $validated[$key] = (bool) $value;
                    break;
                case "login_protect_max_attempts":
                    $validated[$key] = in_array(
                        absint($value),
                        [1, 2, 3, 4, 5, 6],
                        true
                    )
                        ...
                        : 3;
                    break;
                case "login_protect_block_duration":
                    $validated[$key] = in_array(
                        absint($value),
                        [1, 3, 6, 12, 24, 48],
                        true
                        ...
                        : 6;
                    break;
                case "custom_trusted_proxies":
                    if (is_array($value)) {
                        $validated[$key] = array_values(
                            array_unique(
                                array_filter($value, function ($ip) {
                                    return OPTISTATE_Utils::validate_ip_or_cidr(
                                        (string) $ip
                                    );
                                })
                            )
                ...
                case "ip_whitelist":
                    ...
                    break;
                case "one_click_extra_items":
                    if (is_array($value)) {
                        $validated[$key] = array_values(
                            array_unique(
                                array_filter($value, function ($k) use (
                                    $all_cleanup_items
                                ) {
                                    return isset($all_cleanup_items[$k]);
                                })
                            )
                        );
                    } else {
    ...
    private function ensure_table_exists(): void
    {
        ...
        $success = OPTISTATE_Utils::create_table_if_not_exists(
            $table_name,
            $sql,
            true
        );
    ...
    public function check_user_access(): bool
    {
        $error_message = __(
        ...
        if (!current_user_can("manage_options")) {
            if (defined("DOING_AJAX") && DOING_AJAX) {
                OPTISTATE_Utils::send_json_error($error_message, 403);
            } else {
        ...
        if (!in_array($current_user_id, $allowed_users, true)) {
            $restricted_msg = __(
                "<strong>Access Restricted: Your administrator account is not on the allowed list for this plugin.</strong><br>✉︎ Please contact the system administrator to request access.",
                "optistate"
            );
            if (defined("DOING_AJAX") && DOING_AJAX) {
                OPTISTATE_Utils::send_json_error($restricted_msg, 403);
            } else {
                wp_die($restricted_msg, 403);
            }
            return false;
        }
        return true;
    }
    public function cleanup_deleted_user_from_access_list(int $user_id): void
    {
        $settings = $this->get_persistent_settings();
        $allowed_users = $settings["allowed_users"] ?? [];
        if (empty($allowed_users)) {
    ...
    public function handle_settings_download(): void
    {
        if (
            !isset($_GET["action"]) ||
            $_GET["action"] !== "optistate_download_settings"
        ) {
            return;
        }
        ...
        if (
            ...
            !wp_verify_nonce(
                ...
                "optistate_download_settings"
            )
        ) {
            wp_die(
                esc_html__("Security check failed.", "optistate"),
                esc_html__("Security Error", "optistate"),
                ["response" => 403]
            );
        }
        if (!current_user_can("manage_options")) {
        ...
        try {
            ...
            if (empty($settings)) {
                wp_die(
                    esc_html__("No settings found to export.", "optistate"),
                    esc_html__("Export Error", "optistate"),
                    ["response" => 500]
                );
            }
            ...
            $export_data = [
                ...
                "version" => OPTISTATE::VERSION,
                "exported_at" => current_time("Y-m-d H:i:s"),
                "exported_timestamp" => time(),
                "site_url" => get_site_url(),
                "wp_version" => get_bloginfo("version"),
                "settings" => $settings,
                "cron_manager_state" => $cron_state,
            ];
            ...
            if ($json_content === false) {
                wp_die(
                    esc_html__(
                        "Failed to generate settings file.",
                        "optistate"
                    ),
                    esc_html__("Export Error", "optistate"),
                    ["response" => 500]
            ...
            $this->main_plugin->log_entry(
                "📥 " . __("Settings Exported by {username}", "optistate")
            );
            ...
            $filename = "optistate-settings-" . gmdate("Y-m-d-His") . ".json";
            ...
            OPTISTATE_Utils::log_critical_error(
                "handle_settings_download failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            wp_die(
                esc_html__(
    ...
    public function ajax_save_max_backups(): void
    {
        if (!$this->guard_save_request()) {
            return;
        }
        try {
            ...
            if ($old_max_backups !== $max_backups) {
                $this->main_plugin->log_entry(
                    "⚙️ " .
                        __(
                            "Maximum Backups to Keep Updated by {username}",
                            "optistate"
                        )
                );
            }
            ...
            OPTISTATE_Utils::log_critical_error(
                "ajax_save_max_backups failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            OPTISTATE_Utils::send_json_error(
                __(
    ...
    public function ajax_export_settings(): void
    {
        ...
        try {
            $download_url = add_query_arg(
                [
                    "action" => "optistate_download_settings",
            ...
            OPTISTATE_Utils::send_json_success([
                ...
                "message" => __("Settings export ready.", "optistate"),
            ]);
        } catch (\Throwable $e) {
            OPTISTATE_Utils::log_critical_error(
                "ajax_export_settings failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            OPTISTATE_Utils::send_json_error(
                __(
                    "An unexpected error occurred while preparing the export.",
                    "optistate"
                )
            );
        }
    ...
    public function ajax_import_settings(): void
    {
        ...
        try {
            if (
                !isset($_FILES["settings_file"]) ||
                !is_uploaded_file($_FILES["settings_file"]["tmp_name"])
            ) {
                OPTISTATE_Utils::send_json_error(
                    __("No file uploaded.", "optistate")
                );
                return;
            }
            ...
            if ($file["error"] !== UPLOAD_ERR_OK) {
                OPTISTATE_Utils::send_json_error(
                    __("File upload error.", "optistate")
                );
                return;
            }
            if ($file["size"] > 1048576) {
                OPTISTATE_Utils::send_json_error(
                    __("File is too large. Maximum size is 1MB.", "optistate")
                );
                return;
            }
            ...
            try {
                ...
                if ($finfo) {
                    ...
                    if (!in_array($mime_type, $allowed_mimes, true)) {
                        OPTISTATE_Utils::send_json_error(
                            __("Invalid file format detected.", "optistate")
                        );
                        return;
                    }
                }
            ...
            if ($json_content === false) {
                OPTISTATE_Utils::send_json_error(
                    __("Failed to read uploaded file.", "optistate")
                );
                return;
            }
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                OPTISTATE_Utils::send_json_error(
                    __("Invalid JSON file. Error: ", "optistate") .
                        json_last_error_msg()
                );
                return;
            }
            if (
                ...
                OPTISTATE_Utils::send_json_error(
                    __("Invalid settings file structure.", "optistate")
                );
                return;
            }
            if ($import_data["plugin"] !== "WP Optimal State") {
                OPTISTATE_Utils::send_json_error(
                    __(
                        "This file is not a valid WP Optimal State settings export.",
                        "optistate"
                    )
                );
                return;
            ...
            if (!is_array($new_settings)) {
                OPTISTATE_Utils::send_json_error(
                    __("Settings data is corrupted.", "optistate")
                );
                return;
            }
            ...
            if (!empty($validated_settings["ip_block_list"])) {
                $current_user_ip = OPTISTATE_Utils::get_client_ip(
                    ...
                    $validated_settings["custom_trusted_proxies"] ?? []
                );
                $is_self_blocked = false;
                foreach ($validated_settings["ip_block_list"] as $rule) {
                    if (OPTISTATE_Utils::ip_in_range($current_user_ip, $rule)) {
                ...
                if ($is_self_blocked) {
                    OPTISTATE_Utils::send_json_error(
                        sprintf(
                            __(
                                "Import aborted: Your current IP (%s) is in the blocked list. Please remove it from the imported list and try again.",
                                "optistate"
                            ),
            ...
            if (!$success) {
                OPTISTATE_Utils::send_json_error(
                    __("Failed to save imported settings.", "optistate")
                );
                return;
            }
            if (
                ...
                $this->main_plugin->login_protection->create_table();
                global $wpdb;
                $table_name =
                    $wpdb->prefix . OPTISTATE_Login_Protection::TABLE_NAME;
                if (OPTISTATE_Utils::table_exists($table_name)) {
                    $ips = $wpdb->get_col("SELECT ip_address FROM $table_name");
                    if (!empty($ips)) {
                        $wpdb->query(
                            $wpdb->prepare(
                                "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
                                "_transient_optistate_block_%"
                            )
                        );
                        $wpdb->query(
                            $wpdb->prepare(
                                "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
                                "_transient_optistate_clean_%"
                            )
                        );
                    }
                }
            }
            if (isset($validated_settings["ip_block_list"])) {
                $this->main_plugin->login_protection->restore_ip_block_list(
            ...
            $this->main_plugin->invalidate_plugin_caches();
            $this->main_plugin->log_entry(
                "📤 " . __("Settings Imported by {username}", "optistate")
            );
            if (!class_exists("OPTISTATE_Presets")) {
                require_once OPTISTATE_INCLUDES_DIR .
                    "class-optistate-presets.php";
            }
            $preset_key = $validated_settings["last_applied_preset"] ?? "";
            $preset_label = "";
            if (!empty($preset_key)) {
                $presets = OPTISTATE_Presets::get_presets();
                $preset_label = isset($presets[$preset_key])
            ...
            $summary = [
                ...
                "auto_optimize_days" =>
                    $validated_settings["auto_optimize_days"],
                "email_notifications" =>
                    $validated_settings["email_notifications"],
                "performance_features_count" => count(
                    $validated_settings["performance_features"]
                ),
                "imported_from_site" => isset($import_data["site_url"])
                    ? esc_url($import_data["site_url"])
                    : __("Unknown", "optistate"),
                "exported_at" => isset($import_data["exported_at"])
            ...
            OPTISTATE_Utils::log_critical_error(
                "ajax_import_settings failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            OPTISTATE_Utils::send_json_error(
                __(
                    "An unexpected error occurred while importing settings.",
                    "optistate"
                )
            );
        }
    }
    public function ajax_save_user_access(): void
    {
        if (!$this->guard_save_request()) {
            return;
        }
        try {
            ...
            if (empty($user_ids)) {
                ...
                $valid_ids = array_map("intval", wp_list_pluck($users, "ID"));
                $allowed_users = array_values(array_unique($valid_ids));
            }
            $current_user_id = get_current_user_id();
            if (
                !empty($allowed_users) &&
                !in_array($current_user_id, $allowed_users, true)
            ) {
                OPTISTATE_Utils::send_json_error(
                    __(
                        "⚠️ Warning: You are about to lock yourself out! Your account is not in the allowed users list. Please include yourself before saving.",
                        "optistate"
                    ),
                    400,
                    ["lock_out_warning" => true]
                );
                return;
            }
            $old_settings = $this->get_persistent_settings();
            $old_allowed_users = $old_settings["allowed_users"] ?? [];
            $success = $this->save_persistent_settings([
                "allowed_users" => $allowed_users,
            ]);
            if ($success) {
                $added_users = array_diff($allowed_users, $old_allowed_users);
                $removed_users = array_diff($old_allowed_users, $allowed_users);
                $audit_message = "";
                if (!empty($added_users)) {
                    $audit_message .= sprintf(
                        "👤 User Access Updated by {username} | Added: %s user(s)",
                        number_format_i18n(count($added_users))
                    );
                }
                if (!empty($removed_users)) {
                    $audit_message .= sprintf(
                        "👤 User Access Updated by {username} | Removed: %s user(s)",
                        number_format_i18n(count($removed_users))
                    );
                }
                if (!empty($audit_message)) {
                    $this->main_plugin->log_entry($audit_message);
                }
                $count = count($allowed_users);
                $message =
                    $count === 0
                        ? __(
                            "All administrators can now access the plugin.",
                            "optistate"
                        )
                        : sprintf(
                            _n(
                                "Access restricted to %s administrator.",
                                "Access restricted to %s administrators.",
                                $count,
                                "optistate"
                            ),
                ...
                OPTISTATE_Utils::send_json_error(
                    __("Failed to save user access settings.", "optistate")
                );
            }
        } catch (\Throwable $e) {
            OPTISTATE_Utils::log_critical_error(
                "ajax_save_user_access failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            OPTISTATE_Utils::send_json_error(
                __(
                    "An unexpected error occurred while saving user access.",
                    "optistate"
                )
            );
        }
    }
    public function secure_file_write_atomic(
        string $filepath,
        string $content,
        bool $require_csrf = true
    ): bool {
        $is_system_context = wp_doing_cron() || (defined("WP_CLI") && WP_CLI);
        if (!$is_system_context && !current_user_can("manage_options")) {
            return false;
        }
        if ($require_csrf && wp_doing_ajax()) {
        ...
        $wordpress_root = rtrim(get_home_path(), "/\\");
        $normalized_path = str_replace("\\", "/", $filepath);
        $normalized_root = str_replace("\\", "/", $wordpress_root);
        if (
            basename($normalized_path) !== ".htaccess" ||
            dirname($normalized_path) !== $normalized_root
        ) {
            return false;
        }
        $fs = $this->main_plugin->get_filesystem();
        if (!$fs || !$fs instanceof WP_Filesystem_Base) {
            return false;
        }
        try {
            $temp_file = $filepath . ".tmp." . bin2hex(random_bytes(8));
            if (
                $fs->put_contents($temp_file, $content, FS_CHMOD_FILE) === false
            ) {
                OPTISTATE_Utils::log_critical_error(
                    "Atomic write: put_contents failed",
                    [
                        "temp_file" => $temp_file,
                        "filepath" => $filepath,
                        "filesystem_method" => $fs->method,
                    ]
                );
                return false;
            }
            $fs->chmod($temp_file, 0644);
            $backup = $filepath . ".bak";
            if ($fs->exists($filepath)) {
                if (!$fs->copy($filepath, $backup, true)) {
                    OPTISTATE_Utils::log_critical_error(
                        "Atomic write: backup copy failed",
                        ["filepath" => $filepath, "backup" => $backup]
                    );
                    $fs->delete($temp_file);
                    return false;
                }
            }
            if (!$fs->move($temp_file, $filepath, true)) {
                OPTISTATE_Utils::log_critical_error(
                    "Atomic write: rename failed",
                    ["temp_file" => $temp_file, "target" => $filepath]
                );
                if ($fs->exists($backup)) {
                    $fs->move($backup, $filepath, true);
                }
            ...
            OPTISTATE_Utils::log_critical_error(
                "secure_file_write_atomic exception: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            return false;
        }
    ...
    public function ajax_save_one_click_extra_items(): void
    {
        ...
        try {
            ...
            $one_click_backup =
                isset($_POST["one_click_backup"]) &&
                $_POST["one_click_backup"] === "1";
            $current_settings = $this->get_persistent_settings();
            $old_extra = $current_settings["one_click_extra_items"] ?? [];
            $new_extra = $items;
            sort($old_extra);
            sort($new_extra);
            $extra_changed = $old_extra !== $new_extra;
            $old_backup =
            ...
            if ($success) {
                if ($extra_changed || $backup_changed) {
                    $this->main_plugin->log_entry(
                        "💥 " .
                            __(
                                "One-Click configuration updated by {username}",
                                "optistate"
                            )
                    );
                }
                OPTISTATE_Utils::send_json_success([
                    "message" => __(
                        "One-Click configuration saved successfully.",
                        "optistate"
                    ),
                ]);
            } else {
                OPTISTATE_Utils::send_json_error(
                    __("Failed to save One-Click configuration.", "optistate")
                );
            }
        } catch (\Throwable $e) {
            OPTISTATE_Utils::log_critical_error(
                "ajax_save_one_click_extra_items failed: " . $e->getMessage(),
                ["trace" => $e->getTraceAsString()]
            );
            OPTISTATE_Utils::send_json_error(
                __(
