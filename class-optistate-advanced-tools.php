...
class OPTISTATE_Advanced_Tools
{
    ...
    public function ajax_get_table_analysis(): void
    {
        ...
        try {
            ...
            if (empty($tables)) {
                OPTISTATE_Utils::send_json_error(
                    __("Failed to retrieve table information", "optistate")
                );
                return;
            }
            $analysis = [
    ...
    public function ajax_run_analyze_repair_chunk(): void
    {
        ...
        try {
            ...
            try {
                switch ($state["current_step"]) {
                    ...
                    case "done":
                        ...
                        $this->main_plugin->log_entry($log_msg);
            ...
            OPTISTATE_Utils::send_json_error(
                __("An unexpected error occurred.", "optistate")
            );
        } finally {
    ...
    private function get_autoload_candidates(): array
    {
        ...
        $fetch_batch_size = $use_cli ? 2000 : 1000;
        $last_seen_id = 0;
        while (true) {
    ...
    public function ajax_preview_autoload_options(): void
    {
        ...
        try {
            ...
            OPTISTATE_Utils::send_json_error(
                __("Could not fetch preview.", "optistate")
            );
        }
    }
    public function ajax_restore_autoload_backup(): void
    {
        ...
        try {
            ...
            if (!$backup || !is_array($backup)) {
                OPTISTATE_Utils::send_json_error(
                    __("No autoload backup found to restore.", "optistate")
                );
                return;
            }
            ...
            if ($count === false) {
                OPTISTATE_Utils::send_json_error(
                    __("Failed to restore autoload backup.", "optistate")
                );
                return;
            }
    ...
    public function ajax_optimize_autoload(): void
    {
        ...
        try {
            ...
            OPTISTATE_Utils::send_json_error($error_message, 500, $results);
        }
    }
                ...
                function () {
                    return OPTISTATE_Utils::with_stats_expiry_disabled(
                        function () {
                            ...
                            $query = $wpdb->prepare(
                                "SELECT /*+ MAX_EXECUTION_TIME(30000) */ TABLE_NAME, INDEX_NAME as Key_name, SEQ_IN_INDEX as Seq_in_index, COLUMN_NAME as Column_name, NON_UNIQUE as Non_unique FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = %s",
                                DB_NAME
                            );
    ...
    public function run_index_chunk_worker(string $task_id): void
    {
        ...
        try {
            ...
            $is_actually_done =
                $type === "add" ? !empty($check) : empty($check);
            if ($is_actually_done) {
                $task["status"] = "done";
                $this->process_store->set(
                    $task_id,
    ...
    public function ajax_scan_integrity(): void
    {
        ...
        try {
            ...
            $union_sql =
                "SELECT /*+ MAX_EXECUTION_TIME(30000) */ rule_type, cnt FROM (" .
    ...
    private function fallback_integrity_scan(
        ...
        foreach ($rules as $type => $rule) {
            ...
            $extra_where = isset($rule["extra_where"])
                ? $rule["extra_where"]
                : "";
            $sql = "SELECT /*+ MAX_EXECUTION_TIME(30000) */ COUNT(*) FROM $child_table c LEFT JOIN $parent_table p ON c.{$rule["child_key"]} = p.{$rule["parent_key"]} WHERE p.{$rule["parent_key"]} IS NULL $extra_where";
    ...
    public function ajax_fix_integrity(): void
    {
        ...
        try {
            ...
            try {
                ...
                OPTISTATE_Utils::send_json_error([
                    "message" => __("Database error during fix.", "optistate"),
                ]);
                return;
            }
