...
class OPTISTATE_Utils
{
    ...
    public static function detect_server_type(): string
    {
        ...
        if (stripos($software, "nginx") !== false) {
            ...
            stripos($software, "apache") !== false ||
            function_exists("apache_get_modules")
        ) {
            $server_type = "apache";
        } elseif (
            isset($_SERVER["HTTP_X_LITESPEED_CACHE"]) ||
    ...
    public static function get_mysql_version(): string
    {
        if (self::$mysql_version_cache === null) {
            ...
            self::$mysql_version_cache = (string) $wpdb->get_var(
                "SELECT VERSION()"
            );
        }
    ...
    public static function get_table_status(
        ...
        if (
            !$force &&
            array_key_exists($table_name, self::$table_statuses_cache)
        ) {
    ...
    public static function clear_table_existence_cache(
    ...
    public static function create_table_if_not_exists(
        ...
        $exists = (bool) self::get_or_set_transient(
            ...
            static function () use ($table_name, $sql, $use_dbdelta): bool {
                ...
                if ($use_dbdelta) {
                    ...
                    try {
                        ...
                        self::log_critical_error(
                            "dbDelta threw an exception for table {$table_name}: " .
                                $e->getMessage()
                        );
                ...
                self::log_critical_error("Table creation failed", [
                    "table" => $table_name,
    ...
    public static function generate_safe_table_name(
        string $original,
        string $prefix,
        int $max_length = 64
    ): string {
    ...
    private static function build_session_var_statement(
        string $var,
        $value
    ): ?string {
        if (!self::is_valid_session_var_name($var)) {
            self::log_critical_error(
                "Rejected malformed session variable name",
                ["name" => $var]
            );
        ...
        if ($literal === null) {
            self::log_critical_error("Rejected unsafe session variable value", [
                "name" => $var,
            ]);
    ...
    public static function with_session_vars(
        array $vars,
        callable $callback,
        $db = null
    ) {
        ...
        if ($db === null) {
            foreach ($vars as $var => $value) {
                $statement = self::build_session_var_statement(
                    (string) $var,
                    $value
                );
            ...
            try {
                ...
                foreach ($originals as $var => $original) {
                    ...
                    $restore = self::build_session_var_statement(
                        (string) $var,
                        $original
                    );
        ...
        foreach ($vars as $var => $value) {
            ...
            try {
                ...
                self::log_critical_error(
                    "Failed to set session variable {$var}: " . $e->getMessage()
                );
            }
        }
        ...
        try {
            ...
            foreach ($originals as $var => $original) {
                ...
                try {
                    ...
                    self::log_critical_error(
                        "Failed to restore session variable {$var}: " .
                            $e->getMessage()
                    );
                }
            }
        ...
        if (version_compare(self::get_mysql_version(), "8.0.0", ">=")) {
            ...
            if ($var_exists) {
                try {
                    ...
                    self::log_critical_error(
                        "Failed to set stats_expiry: " . $e->getMessage()
                    );
                    $modified = false;
                }
            }
        ...
        try {
            ...
            if ($modified && $orig_expiry !== null) {
                try {
                    ...
                    self::log_critical_error(
                        "Failed to restore stats_expiry: " . $e->getMessage()
                    );
                }
            }
        }
    ...
    public static function without_foreign_key_checks(
        callable $callback,
        $db = null
    ) {
        return self::with_session_vars(
    ...
    public static function transaction(callable $callback, $db = null)
    {
        ...
        if ($db === null) {
            if ($wpdb->query("START TRANSACTION") === false) {
                throw new \Exception(
                    "Failed to start transaction on wpdb: " . $wpdb->last_error
                );
            }
            ...
            try {
                ...
                if ($wpdb->query("COMMIT") === false) {
                    throw new \Exception(
                        "Failed to commit transaction on wpdb: " .
                            $wpdb->last_error
                    );
                }
        ...
        try {
            ...
            throw new \Exception(
                "Database transaction start failed: " . $e->getMessage()
            );
        }
            ...
            if (!is_string($fallback) || $fallback === "") {
                try {
                    ...
                    $fallback = bin2hex(
                        hash(
                            "sha256",
                            uniqid((string) wp_rand(), true) . time() . DB_NAME,
                            true
                        )
                    );
    ...
    public static function send_json_error(
        $message,
        int $status = 400,
        array $extra = []
    ): void {
        if (is_array($message)) {
            ...
            $message =
                $extra["message"] ?? __("An error occurred.", "optistate");

            unset($extra["message"]);
        }

        $response = [
            "message" => self::sanitize_error_message((string) $message),
        ];
    ...
    public static function send_json_success(
        array $data = [],
        int $status = 200
    ): void {
    ...
    public static function get_or_set_transient(
        string $key,
        callable $callback,
        $expiration = 0,
        bool $force = false
        ...
        try {
            ...
            self::log_critical_error(
                "get_or_set_transient callback failed for key '{$key}': " .
                    $e->getMessage()
            );
    ...
    public static function log_critical_error(
        string $message,
        array $context = []
    ): void {
        ...
        if (!empty($context)) {
            ...
            if (isset($context["backup_file"])) {
                $safe_context["backup_file"] = basename(
                    (string) $context["backup_file"]
                );
            }
            ...
            $safe_context["memory_usage"] = size_format(
                memory_get_usage(true),
                2
            );
            $safe_context["peak_memory"] = size_format(
                memory_get_peak_usage(true),
                2
            );

            if (defined("WP_START_TIMESTAMP")) {
                $safe_context["exec_time_sec"] = round(
    ...
    public static function get_folder_size(
        ...
        $sensitive_pattern =
            '/\.(sql|backup|log|key|pem|crt|env|bak|old|db|sqlite|gz)$' .
        ...
        try {
            ...
            foreach ($iterator as $file) {
                ...
                if (
                    $check_sensitive &&
                    !$stats["sensitive"] &&
                    preg_match($sensitive_pattern, $file->getFilename())
                ) {
            ...
            self::log_critical_error(
                "Failed to scan folder stats: " . $e->getMessage(),
                ["path" => $path]
            );
        }
    ...
    public static function local_time_to_timestamp(string $time_string): int
    {
        try {
            ...
            self::log_critical_error("local_time_to_timestamp failed", [
    ...
    public static function format_timestamp($timestamp): string
    {
        ...
        $format =
            self::get_cached_option("date_format") .
            " " .
            self::get_cached_option("time_format");
    ...
    public static function get_client_ip(
        ...
        $trusted_cache_key =
            ...
            "|" .
            implode(",", $trusted_proxies);

        if (!isset(self::$trusted_proxy_cache[$trusted_cache_key])) {
            $default_trusted = ["127.0.0.1", "::1"];

            if ($cloudflare_enabled) {
                $default_trusted = array_merge(
                    $default_trusted,
                    self::get_cloudflare_ip_ranges()
                );
            }

            self::$trusted_proxy_cache[
                $trusted_cache_key
            ] = (array) apply_filters(
        ...
        $cache_key =
            ($cloudflare_enabled ? "1" : "0") . "|" . implode(",", $trusted);
    ...
    private static function resolve_forwarded_ip(
        string $header,
        array $trusted,
        string $fallback
    ): string {
    ...
    private static function ipv6_in_range(string $ip, string $range): bool
    {
        ...
        if (
            $bytes > 0 &&
            substr($ip_packed, 0, $bytes) !== substr($subnet_packed, 0, $bytes)
        ) {
            return false;
        }
        ...
        if ($remaining_bits > 0) {
            ...
            if (
                (ord($ip_packed[$bytes]) & $mask) !==
                (ord($subnet_packed[$bytes]) & $mask)
            ) {
                return false;
            }
        }
    ...
    public static function get_bot_rules(
        string $user_agents_string,
        bool $include_whitelist_bypass = false
    ): string {
        ...
        $cache_key = md5(
            $user_agents_string . ($include_whitelist_bypass ? "1" : "0")
        );
        ...
        $rules[] = "";
        $rules[] =
            "  # Allow access to admin area and login page (prevent admin lockout)";
        ...
        $rules[] =
            '  SetEnvIf Request_URI "^/wp-content/.*\.(css|js|png|jpg|jpeg|gif|webp|svg|woff|woff2|ttf|eot)$" allow_admin';
        $rules[] = "";
...
public static function get_ip_block_rules( string $raw_ips, bool $include_whitelist_bypass = true ): string {
    ...
    public static function get_security_headers_rules(
        bool $optional_headers = false
    ): string {
        if (
            self::$security_headers_rules_cache !== null &&
            !$optional_headers
        ) {
    ...
    public static function remove_tinymce_emoji($plugins): array
    {
        return is_array($plugins)
            ? array_values(array_diff($plugins, ["wpemoji"]))
            : [];
    }
    ...
    public static function font_opt_remove_google_fonts(): void
    {
        ...
        foreach ($wp_styles->queue as $handle) {
            ...
            if (
                strpos((string) $src, "fonts.googleapis.com") !== false ||
    ...
    public static function add_async_decoding(
        $filtered_image,
        $context,
        $attachment_id
    ): string {
        ...
        $result = preg_replace_callback(
            ...
            static function ($matches) use (
                ...
                $has_decoding = preg_match(
                    "/\bdecoding\s*=/i",
                    $original_attributes
                );
                $has_loading = preg_match(
                    "/\bloading\s*=/i",
                    $original_attributes
                );
                $has_fetchpriority = preg_match(
                    "/\bfetchpriority\s*=/i",
                    $original_attributes
                );

                $is_priority =
                    $image_processed_count <= self::LCP_IMAGE_THRESHOLD;
        ...
        switch ($mode) {
            case "slow":
                add_filter("heartbeat_settings", static function ($settings) {
                    $settings["interval"] = max(
                        (int) ($settings["interval"] ?? 0),
                        120
                    );
    ...
    public static function cache_query_results($posts, $query)
    {
        ...
        if (
            count($posts) >
            (int) (self::$query_cache_config["max_cache_size"] ?? 500)
        ) {
            return $posts;
        }
        ...
        wp_cache_set(
            ...
            [
                "posts" => $posts,
                "found_posts" => isset($query->found_posts)
                    ? (int) $query->found_posts
                    : count($posts),
                "max_num_pages" => isset($query->max_num_pages)
    ...
    private static function should_cache_query($query): bool
    {
        ...
        if (
            isset($query->query_vars["cache_results"]) &&
            false === $query->query_vars["cache_results"]
        ) {
            return false;
        }
        ...
        return (bool) apply_filters(
            "optistate_should_cache_query",
            true,
            $query
        );
    }

    public static function generate_query_cache_key(
        $query,
        string $query_type
    ): string {
        $relevant_keys = [
